...
writeRegex = do
     ...
     case regex of
          ...
          (Right r) -> do
                 putStrLn "Right"
                 -- matches <- match r " "
                 possibleBytestring <- serializeJitRegexs [r]
...
readRegex :: IO ()
readRegex = do
          regexData <- B.readFile "test.regexs"
          possibleRegexs <- deserializeJITRegexs regexData 1
          case possibleRegexs of
               (Left error) -> do
                     putStrLn "Deserialization Error"
                     putStrLn $ show error
